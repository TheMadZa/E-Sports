...
BEGIN
    OPEN c_obtenerInfo for
        SELECT nom_equipo,COUNT(DISTINCT s.id_staff) AS "cantidadStaff",COUNT(DISTINCT j.id_jugador) AS "cantidadJugadores"
        FROM EQUIPO e
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('No se encontró información para el equipo especificado.');
    WHEN OTHERS THEN
